...
def figure_1(graph_dict, figures_dir, n=10):
    ...
    """
    # Sort the file list so everything's in alphabetical order
    f_list.sort()
    f_list = f_list[:4] + f_list[8:] + f_list[4:8]
    fig = plt.figure(figsize=(20, 9), facecolor='white')

    grid = gridspec.GridSpec(3, 4)
    grid.update(left=0.1, right=1, top=0.95, bottom=0, wspace=0, hspace=0.05)
    ...
    for g_loc, f in zip(grid, f_list):
        ...
        cost = f.split('_covar_')[1].split('_COST_')[1]
        cost = cost.rstrip('.png')
        if 'degree' in f:
            ax.text(0.9, 1, 
                        'density = {:.0f}%'.format(np.float(cost)),
                        ...
                        fontsize=20,
                        weight='bold')
        if ('COST_05' in f) and ('degree' in f):
            ax.text(-0.15, 0.5, 
                        'Degree distribution'.format(np.float(cost)),
                        ...
                        fontsize=20,
                        weight='bold')
        if ('COST_05' in f) and ('rich' in f):
            ax.text(-0.15, 0.5, 
                        'Rich club'.format(np.float(cost)),
                        horizontalalignment='left',
                        verticalalignment='center',
                        transform=ax.transAxes,
                        rotation='vertical',
                        fontsize=20,
                        weight='bold')
        if ('COST_05' in f) and ('network' in f):
            ax.text(-0.15, 0.5, 
                        'Network Measures'.format(np.float(cost)),
                        horizontalalignment='left',
                        verticalalignment='center',
                        transform=ax.transAxes,
                        rotation='vertical',
